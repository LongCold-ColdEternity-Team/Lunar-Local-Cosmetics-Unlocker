...
- 兼容部分客户端扩展管理器缺失或目录未加载的情况：这些分类会记录为跳过，不再阻塞 Cosmetics 主饰品解锁。
...
3. UI 检测到 `Lunar Client 1.8.9` 后，点击“注入并解锁”。界面会等待代理完成运行时校验，普通饰品与可用扩展分类验证成功后显示“注入成功”；客户端不提供的扩展分类会跳过并记录到日志。
